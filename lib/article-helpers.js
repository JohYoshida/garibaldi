...
module.exports = {
  getArticle: (req, res) => {
    ...
    // Get user's articles
    knex("articles").where({user_id: cookies.user}).then(userArticles => {
      // Get public articles
      knex("articles").whereNot({user_id: cookies.user}).where({is_private: false}).then(publicArticles => {
        // Get target article
        knex("articles")
          ...
          .then(article => {
            ...
            // Render article
            res.render("show", { userArticles, publicArticles, article, cookies });
          });
      })
    });
  },
      ...
      knex("articles").whereNot({user_id: cookies.user}).where({is_private: false}).then(publicArticles => {
        ...
        knex("articles")
          ...
          .then(article => {
            ...
            normalizeArticle(article);
            // Render article
            res.render("edit", { userArticles, publicArticles, article, cookies });
          });
      })
    });
  },
